...
the shell the picker overlay; `setup` installs the `plex-float` command, the
window rules, and checks you have mpv. The shell caches plugin QML once it has
loaded it, so it needs the restart to notice a picker that was not there before.
